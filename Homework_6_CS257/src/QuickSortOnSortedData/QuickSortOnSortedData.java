...
// modified by spc
// main method modified by Roshan Poudel

package QuickSortOnSortedData;

import java.util.Scanner;

public class QuickSortOnSortedData
{
    static int compares;
    static int swaps;
	...
	callStack.push(new QuickSortOnSortedData.callFrame(0,n-1));
	while (!callStack.isEmpty())
	{   // some "virtual" method outstanding
	    QuickSortOnSortedData.callFrame curr = (QuickSortOnSortedData.callFrame)callStack.get();
		...
		callStack.push(new QuickSortOnSortedData.callFrame(curr.low,curr.pivot-1));
	    } else if (curr.PC == 2) { // sort upper
		curr.PC++;
		// sort the larger values....
		callStack.push(new QuickSortOnSortedData.callFrame(curr.pivot+1,curr.high));
